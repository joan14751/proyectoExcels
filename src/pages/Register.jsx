...
export default function UploadExcel() {
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      const result = await excelService.processExcel(selectedFile);
      setPreview(result);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await excelService.uploadAndSave(file, autoNombre, tipo);
      setSuccess(true);
      setFile(null);
      setPreview(null);
    } catch (err) {
                ...
                <form onSubmit={handleSubmit}>
                  ...
                  <div className="mb-5">
                    ...
                    >
                      ...
                      <h5>Haz clic para seleccionar el archivo</h5>
                      <p className="text-muted">.xlsx o .xls</p>
                      <input 
                        id="fileInput" 
                        type="file" 
                        className="d-none" 
                  ...
                  {preview && <div className="alert alert-success">✅ {preview.rowCount} filas detectadas</div>}
                  {error && <div className="alert alert-danger">{error}</div>}
                  {success && <div className="alert alert-success fw-bold">¡Archivo subido correctamente!</div>}
