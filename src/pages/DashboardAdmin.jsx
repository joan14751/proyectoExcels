...
export default function DashboardAdmin() {
  ...
  // Mejora en la extracción del path del archivo
  ...
  const handlePreview = async (doc) => {
    try {
      ...
      // Obtenemos los datos con encabezados para asegurar que se vea todo
  ...
  // CORRECCIÓN: Descarga directa vía Blob para evitar bloqueos
  const handleDownload = async (doc) => {
    ...
    try {
      ...
      // Crear URL de objeto para el Blob descargado
    ...
    <>
      ...
      <div className="container py-5">
        <header className="mb-5">
            ...
            <p className="text-muted">Gestión de archivos Excel y Usuarios</p>
        </header>
        ...
        <div className="row g-4 mb-5">
          <div className="col-md-4">
            <Link to="/admin/upload" className="text-decoration-none">
              <div className="card h-100 shadow-sm border-0 transition-hover" style={{ background: '#22c55e', color: 'white' }}>
                <div className="card-body text-center py-5">
                  <Upload size={48} className="mb-3" />
                  <h5 className="fw-bold">Subir Excel</h5>
                  <p className="small opacity-75 mb-0">IMS, Stock, Rotación</p>
                </div>
              </div>
            </Link>
          </div>
          <div className="col-md-4">
            <Link to="/admin/users" className="text-decoration-none">
              <div className="card h-100 shadow-sm border-0 transition-hover" style={{ background: '#3b82f6', color: 'white' }}>
                <div className="card-body text-center py-5">
                  <Users size={48} className="mb-3" />
                  <h5 className="fw-bold">Usuarios</h5>
                  <p className="small opacity-75 mb-0">Gestionar cuentas</p>
                </div>
              </div>
            </Link>
          </div>
          <div className="col-md-4">
            <Link to="/admin/permissions" className="text-decoration-none">
              <div className="card h-100 shadow-sm border-0 transition-hover" style={{ background: '#f59e0b', color: 'white' }}>
                <div className="card-body text-center py-5">
                  <Key size={48} className="mb-3" />
                  <h5 className="fw-bold">Permisos</h5>
                  <p className="small opacity-75 mb-0">Control de acceso</p>
                </div>
              </div>
            </Link>
          </div>
        ...
        <div className="card shadow-sm border-0 rounded-4 overflow-hidden">
          <div className="card-header bg-white py-3">
            <h5 className="mb-0 fw-semibold text-dark">📋 Lista de Excels Subidos</h5>
          </div>
            ...
            <table className="table table-hover align-middle mb-0">
              ...
              <tbody>
                {loading ? (
                  ...
                  documentos.map(doc => (
                    <tr key={doc.id}>
                      <td className="ps-4 fw-medium">{doc.nombre}</td>
                      <td><span className="badge bg-primary-subtle text-primary px-3">{doc.tipo}</span></td>
                      <td className="text-muted">{new Date(doc.created_at).toLocaleDateString('es-ES')}</td>
                      <td><span className="badge bg-success-subtle text-success px-3">Activo</span></td>
                  ...
                  <thead className="sticky-top bg-white">
                    <tr>
                      {previewData[0]?.map((col, idx) => (
                        <th key={idx} className="p-2 bg-light">{col || `Col ${idx + 1}`}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {previewData.slice(1, 100).map((row, i) => (
                      <tr key={i}>
                        {row.map((cell, j) => <td key={j} className="p-2 text-truncate" style={{ maxWidth: '200px' }}>{cell}</td>)}
                      </tr>
                    ))}
                  </tbody>
                </table>
